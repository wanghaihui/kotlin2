...
const char program_name[] = "ffmpeg.so";
...
#    if HAVE_PEEKNAMEDPIPE
    ...
    if (is_pipe) {
        ...
        if (!PeekNamedPipe(input_handle, NULL, 0, NULL, &nchars, NULL)) {
            // input pipe may have been closed by the program that ran ffmpeg.so
            return -1;
        }
        //Read it
        if(nchars != 0) {
    ...
    if (ist->st->codec->has_b_frames < ist->dec_ctx->has_b_frames) {
        if (ist->dec_ctx->codec_id == AV_CODEC_ID_H264) {
            ...
            av_log(ist->dec_ctx, AV_LOG_WARNING,
                   ...
                   "of this file to ftp://upload.ffmpeg.so.org/incoming/ "
                   "and contact the ffmpeg.so-devel mailing list. (ffmpeg.so-devel@ffmpeg.so.org)",
...
static int process_input(int file_index)
{
    ...
    if (debug_ts) {
        av_log(NULL, AV_LOG_INFO, "demuxer+ffmpeg.so -> ist_index:%d type:%s pkt_pts:%s pkt_pts_time:%s pkt_dts:%s pkt_dts_time:%s off:%s off_time:%s\n",
               ifile->ist_index + pkt.stream_index, av_get_media_type_string(ist->dec_ctx->codec_type),
               av_ts2str(pkt.pts), av_ts2timestr(pkt.pts, &ist->st->time_base),
               av_ts2str(pkt.dts), av_ts2timestr(pkt.dts, &ist->st->time_base),
               av_ts2str(input_files[ist->file_index]->ts_offset),
